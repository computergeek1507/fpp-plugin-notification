...
Telegram::Telegram(std::string const& bot_token, std::string const& chat_id) :
    NotificationBase(),
...
bool Telegram::SendMessage(std::string const& message)
{
    try {
       /*
       curl -s \
       ...
       */
       //https://api.telegram.org/bot<token>/sendMessage?chat_id=<chatId>&text=testing
        std::string repURL = "https://api.telegram.org/bot" + m_bot_token + "/sendMessage";
      ...
      // post_fields = curl_slist_append(post_fields, std::string("token=" + m_token).c_str());
       ...
       LogInfo(VB_PLUGIN, "message %s \n", message.c_str());
        std::string postmsg = "chat_id=" + m_chat_id + "&text=" + message;
